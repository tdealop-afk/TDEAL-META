...
(function () {
  ...
  function render(map) {
    ...
    // 시트에 값이 있으면 그대로, 없으면 계산
    var has = function (k) { return map[k] != null && map[k] !== "" && isFinite(num(map[k])); };
    var totalCost = has("total_cost") ? g("total_cost") : (adSpend + pointsPaid);
    var roasTotalPct = has("roas_total") ? g("roas_total") : (revenue / totalCost) * 100;
    var roasAdPct = has("roas_ad") ? g("roas_ad") : (revenue / adSpend) * 100;
    var cpaTotal = has("cpa_total") ? g("cpa_total") : (totalCost / newConv);
    var pointsHead = has("signups_total") ? g("signups_total") : (pph ? Math.round(pointsPaid / pph) : NaN);
    ...
    // 히어로
    animate("roas-total", roasTotalPct);
    setText("roas-ad", fmt(roasAdPct));
    animate("total-cost", totalCost);
    setText("legend-ad", fmt(adSpend));
    setText("legend-pt", fmt(pointsPaid));
    animate("revenue", revenue);
    setText("roas-mult", isFinite(roasTotalPct) ? (roasTotalPct / 100).toFixed(1) : "—");
    ...
    // 각주
    setText("foot-line1",
      "ROAS는 거래액 ÷ 총비용(광고비+포인트 지급) 기준 " + fmt(roasTotalPct) +
      "%입니다. 광고비만 기준으로는 " + fmt(roasAdPct) + "%입니다.");
    setText("foot-line2",
      "신규 인증 전환(" + fmt(newConv) + "명)은 메타 픽셀 집계 수치이고, 포인트 지급(" +
      fmt(pointsPaid) + "원)은 공유하기 등 간접 유입까지 포함해 신규 가입자 " + fmt(pointsHead) + "명에게 지급됐습니다.");
